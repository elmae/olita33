...
if ( ! function_exists( 'understrap_categories_list' ) ) {
	...
	function understrap_categories_list() {
		...
		if ( $categories_list && understrap_categorized_blog() ) {
			...
			printf( '<span class="cat-links">' . esc_html__( ' | %s | ', 'understrap' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
}
...
if ( ! function_exists( 'understrap_post_nav' ) ) {
	...
	function understrap_post_nav() {
		...
		?>
<nav class="container navigation post-navigation mb-3 py-5 border-top border-bottom">
  <h2 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'understrap' ); ?></h2>
  <div class="d-flex nav-links justify-content-between">
    <?php
				if ( get_previous_post_link() ) {
					previous_post_link( '<span class="nav-previous non-break">%link</span>', _x( '<i class="fa fa-angle-left"></i>&nbsp;%title', 'Previous post link', 'understrap' ) );
				}
				if ( get_next_post_link() ) {
					next_post_link( '<span class="nav-next non-break">%link</span>', _x( '%title&nbsp;<i class="fa fa-angle-right"></i>', 'Next post link', 'understrap' ) );
				}
				?>
  </div><!-- .nav-links -->
</nav><!-- .post-navigation -->
